...
class InvoiceOrderRow {
  final String vin; // رقم شاصي السيارة لهذا الطلب
  final int qty; // عدد الخدمات المنجزة ضمن هذا الطلب
  final double amount; // إجمالي مبلغ هذا الطلب

  InvoiceOrderRow({required this.vin, required this.qty, required this.amount});
}
...
Future<Uint8List> generateMultiOrderInvoicePdf(MultiOrderInvoiceData data) async {
...
pw.Widget _buildHeader(MultiOrderInvoiceData data) {
  return pw.Row(
    crossAxisAlignment: pw.CrossAxisAlignment.start,
        ...
        pw.Padding(
          ...
          child: pw.Row(
            ...
            children: [
              ...
              pw.SizedBox(width: 14),
              pw.Expanded(child: _fieldBlock('ADDRESS', data.customerAddress)),
            ],
          ),
        ),
      ],
...
pw.Widget _buildOrdersTable(MultiOrderInvoiceData data) {
  ...
  for (int i = 0; i < rowCount; i++) {
    final hasData = i < data.orders.length;
    final o = hasData ? data.orders[i] : null;
    rows.add(
      pw.TableRow(
        children: [
          _tableCell((i + 1).toString().padLeft(2, '0'), rowStyle),
          _tableCell(o?.vin ?? '', rowStyle),
          _tableCell(
            hasData ? o!.qty.toString() : '',
            rowStyle,
            align: pw.Alignment.center,
          ),
          _tableCell(
            hasData ? '\$${o!.amount.toStringAsFixed(0)}' : '',
            rowStyle,
            align: pw.Alignment.centerRight,
          ),
        ],
